...
import {
  ...
  MapPin, Briefcase, GraduationCap, Award, Star, Heart, Linkedin, Globe,
  User, FolderOpen, QrCode,
} from "lucide-react";
        ...
        <div className="container max-w-4xl">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 mb-6 print:hidden">
            ...
            <div className="flex items-center gap-2">
              {allowCvDownload && (
                <Button className="rounded-xl gap-2" style={{ background: 'linear-gradient(135deg, #6C5CE7, #00C2FF)' }} onClick={handleExportPDF} disabled={exporting}>
          ...
          <Card className="rounded-2xl overflow-hidden shadow-lg border-0">
            <div ref={cvRef} style={{ width: "100%", minHeight: 800, fontFamily: "'IBM Plex Sans Arabic', sans-serif", backgroundColor: '#fff' }}>
              {/* CV Header */}
              <div className="p-8 pb-6 relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #1E2A78 0%, #3949AB 100%)' }}>
                ...
                <div className="relative flex items-center gap-6" dir="rtl">
                  <div className="shrink-0">
                    {profile.avatar_url ? (
                      <div className="rounded-full p-[3px]" style={{ background: 'linear-gradient(135deg, #6C5CE7, #00C2FF)' }}>
                        <img src={profile.avatar_url} alt="" className="w-[110px] h-[110px] rounded-full object-cover border-4 border-white" />
                      </div>
                    ) : (
                      <div className="w-[110px] h-[110px] rounded-full flex items-center justify-center text-white text-4xl font-bold border-4 border-white/30" style={{ background: 'linear-gradient(135deg, #6C5CE7, #00C2FF)' }}>
                        {initials}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {(profile as any).headline && <p className="text-sm mt-1.5 text-white/80 font-medium">{(profile as any).headline}</p>}
                  ...
                  <div className="shrink-0 bg-white rounded-xl p-2 shadow-sm print:hidden">
                    <QRCodeSVG value={profileUrl} size={64} level="M" fgColor="#1E2A78" />
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-0" dir="rtl" style={{ minHeight: 500 }}>
                ...
                <div className="flex-1 p-6 space-y-5">
                  {profile.summary && (
                    <div>
                      <h2 className="text-sm font-bold mb-2 pb-1.5 border-b-2 flex items-center gap-2" style={{ color: '#1E2A78', borderColor: '#1E2A78' }}>
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        {experiences.map((exp: any, i: number) => (
                          <div key={i} className="pr-3 border-r-2 rounded-sm" style={{ borderColor: '#1E2A7830' }}>
                            ...
                            {exp.company && <p className="text-[11px] font-medium" style={{ color: '#1E2A78' }}>{exp.company}</p>}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {skills.map((s: string, i: number) => (
                          <span key={i} className="rounded-full px-2.5 py-1 text-[11px] font-medium" style={{ backgroundColor: '#EDE7F6', color: '#4A148C' }}>
                            {s}
                          </span>
                        ))}
                      </div>
